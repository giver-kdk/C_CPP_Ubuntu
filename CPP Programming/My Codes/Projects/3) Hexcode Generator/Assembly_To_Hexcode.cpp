...
void init_one_byte_instruction(string oneByteCode[250], int oneByteHex[250]);
...
void asm_to_hex(string instruct, string mnemonic, string operand1, string operand2)
{
	...
	if(byte == 1)
	{
		row = 202;													// There are 202 one-byte instructions for 8085
		cout << "No value in operand1 and operand2" << endl;
		string opCodeArray[250];
		int hexCodeArray[250];
		init_one_byte_instruction(opCodeArray, hexCodeArray);		// Initialize mnemonic and hexcode
		for(int j = 0; j < row; j++)
		{
			if(instruct == opCodeArray[j])
			{
...
void init_one_byte_instruction(string oneByteCode[250], int oneByteHex[250])
{
	// Initialize mnemonics in an array
	oneByteCode[0] = "ADC A";
	oneByteCode[1] = "ADC B";
	oneByteCode[2] = "ADC C";
	oneByteCode[3] = "ADC D";
	...
	// Initialize corerspoding hex code in an array
